...
#     and plugins.manage list rows carry the canonical registry key; toggles are
...
def _stage_session_file_attachment(
    ...
    *,
    ...
    Four cases:
      1. Uploaded ``data_url`` bytes are present — they are authoritative over
         any gateway-local path with the same client-provided name.
      2. The path resolves to a file already INSIDE the session workspace — use
         it as-is (no copy, ``uploaded=False``).
      3. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
      ...
      4. No bytes were supplied and the path doesn't exist on the gateway — fail.
